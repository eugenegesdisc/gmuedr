...
class OgcApiCollectionDataByCorridor(OgcApi):
    ...
    #    return web.Response(status=200)
        api_path = "/collections/{collectionId}/corridor"
        ...
        if best_match == "text/html":
            ...
            the_item_data = self._get_collection_data_by_corridor(
                thecollection_value,
                collection_id, coords, corridor_width,
                width_units, corridor_height, height_units, z, datetime,
                parameter_name, resolution_x, resolution_z, crs, best_match)

        the_final_ret = self._format_get_data_for_corridor(
    ...
    def _get_collection_data_by_corridor(
            self, collection_spec, collection_id, coords, corridor_width,
            width_units, corridor_height, height_units, z=None, datetime=None,
            parameter_name=None, resolution_x=None, resolution_z=None,
            crs=None, f=None):
        try:
            ...
            thedata = theprovider.get_data_for_corridor(
                    collection_id, coords, corridor_width,
                    width_units, corridor_height, height_units, z, datetime,
    ...
    def _format_get_data_for_corridor(
        ...
        if theplugin is None:
            Logger.warning("No formaater for mediatype={} for {}.".format(
                mediatype, "collection_edr_corridor"
            ))
            return thedata
