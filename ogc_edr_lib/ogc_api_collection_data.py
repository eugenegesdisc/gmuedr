...
from ogc_edr_lib.ogc_api_collection_data_by_radius import (
    OgcApiCollectionDataByRadius
)
from ogc_edr_lib.ogc_api_collection_data_by_trajectory import (
    OgcApiCollectionDataByTrajectory
)
from ogc_edr_lib.ogc_api_collection_data_by_corridor import (
    OgcApiCollectionDataByCorridor
)
from ogc_edr_lib.ogc_api_collection_data_by_cube import (
    OgcApiCollectionDataByCube
)
from ogc_edr_lib.ogc_api_collection_data_by_location import (
    OgcApiCollectionDataByLocation
)
...
class OgcApiCollectionData(OgcApi):
    ...
    def get_collection_data_for_location(
        ...
        """
        ocdata = OgcApiCollectionDataByLocation()
    ...
    def get_data_for_cube(
        ...
        Return the data values for the data Cube defined by the query
         parameters
    ...
    #    return web.Response(status=200)
        ocdata = OgcApiCollectionDataByCube()
    ...
    def get_data_for_corridor(
            self, request: web.Request, collection_id, coords, corridor_width,
    ...
    #    return web.Response(status=200)
        ocdata = OgcApiCollectionDataByCorridor()
    ...
    def get_data_for_radius(
            self, request: web.Request, collection_id, coords, within,
        ...
        height intervals, the difference is the number of recurrences is
        defined at the start and the amount to increment the height by is
        defined at the end  i.e. z&#x3D;Rn/min height/height interval  so if
        the request was for 20 height levels 50m apart starting at 100m:
        z&#x3D;R20/100/50
        ...
        * Open intervals: \&quot;2018-02-12T00:00:00Z/..\&quot; or
        ...
        """
        ocdata = OgcApiCollectionDataByRadius()
    ...
    def get_data_for_area(
        ...
        Return the data values for the data area defined by the query
         parameters

        :param collection_id: Identifier (id) of a specific collection
        :type collection_id: str
        :param coords: Only data that has a geometry that intersects the area
         defined by the polygon are selected.  The polygon is defined using a
         ...
         parameter (if crs is not defined the values will be assumed to be
         WGS84 longitude/latitude coordinates).  For instance a polygon that
         roughly describes an area that contains South West England in WGS84
         would look like:
        ...
        :param z: Define the vertical level to return data from
         ...
         queried  z&#x3D;850 or a range to return data for all levels between
         and including 2 defined
         levels i.e. z&#x3D;minimum value/maximum value  for instance if all
         values between and including 10m and 100m  z&#x3D;10/100
         finally a list of
         ...
         z&#x3D;Rn/min height/height interval  so if the request was for 20
         height levels 50m apart starting at 100m:
          z&#x3D;R20/100/50  When not specified data from all available heights
         SHOULD be returned
        :type z: str
         ...
         * Open intervals: \&quot;2018-02-12T00:00:00Z/..\&quot;
          ...
          temporal property that intersects the value of &#x60;datetime&#x60;
          are selected.
         If a feature has multiple temporal properties, it is
        ...
        #x3D;10  would retrieve 10 values along the x-axis from the minimum x
         coordinate to maximum x coordinate (i.e. a value at both the minimum
         x and maximum x coordinates and 8 values between).
        :param resolution_x: Defined it the user requires data at a different
         resolution from the native resolution of the data along the x-axis
         If this is a single value it denotes the number of intervals to
         retrieve data for along the x-axis    i.e. resolution-x&
        :type resolution_x: dict | bytes
        ...
        :param resolution_y: Defined it the user requires data at a different
         ...
         this is a single value it denotes the number of intervals to retrieve
         data for along the y-axis    i.e. resolution-y&
    ...
    def get_data_for_item(
        ...
        Query end point to retrieve data from collection {collectionId} using
         a unique identifier

        :param collection_id: Identifier (id) of a specific collection
        :type collection_id: str
        :param item_id: Retrieve data from the collection using a unique
         identifier.
        :type item_id: str
