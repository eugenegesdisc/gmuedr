...
class EDRProviderXarraysHalfHourly(BaseProvider):
    ...
    type = "provider"
    category = "edr"
    supported_media_types = ["xarrays_imerg_halfhourly"]
    ...
    def _open_data_hdf5files_as_xarray(
            self):
        try:
            thedata = self.provider_def["data"]
            the_files = glob.glob(thedata)
            the_group = None
            if "group" in self.provider_def:
                the_group = self.provider_def["group"]
            # print("the_group=", the_group)
            the_xarrays = list()
            for the_file in the_files:
                # print("the_file=", the_file)
                a_xarray = self._open_hdf5file_as_xarray(the_file, the_group)
                # print("a_xarray=", a_xarray)
                if a_xarray:
                    the_xarrays.append(a_xarray)
            thedata = None
            # print("len arrr=", len(the_xarrays))
            if len(the_xarrays) > 0:
                thedata = xr.combine_by_coords(the_xarrays)
            return thedata
        except Exception as err:
            Logger.warning("In _open_data_hdf5files_as_xarray - error")
            Logger.warning(err)
            return None
    ...
    def list_collection_data_locations(
        ...
        try:
            ...
            # theproperties = self._get_coverage_properties2(thedata)
            print(" In list_collection_data_locations of imerg_halfhouly provider")
            theproperties = self._get_coverage_properties_from_config()
            print("theproperties=", theproperties)
            # the_id_array = self._form_the_id(thedata)
            # thedata["id_user_"] = the_id_array
            # theparams = self._parse_parameter_name(
    ...
    def get_data_for_area(
        ...
        ret_data = None
        try:
            print("IN get_data_for_area - edr_xarrays_imerg_halfhourly")
            # thedata = self.provider_def["data"]
            # if thedata.endswith(".zarr"):
            #    open_func = xr.open_zarr
            # else:
            ...
            # thedata = open_func(thedata)
            thedata = self._open_data_hdf5files_as_xarray()
            print("IN get_data_for_area - edr_xarrays_imerg_halfhourly ... loaded")
            # print("In xarrays daily 010 - area:", thedata)
            theproperties = self._get_coverage_properties2(thedata)
            print("IN get_data_for_area - edr_xarrays_imerg_halfhourly ... proprerties2")
            # print("In xarrays daily 0 - area:", theproperties)
            # the_id_array = self._form_the_id(thedata)
            # print("the_id_array=", the_id_array)
            # thedata["id_user_"] = the_id_array
            theparams = self._parse_parameter_name(
                parameter_name, theproperties)
            # print("In xarrays daily 1 - area:", theparams)
            thedata = thedata.get(theparams)
            print("THEDATA=", thedata)
            if datetime is not None:
                thedata = self._subset_along_time_dimension(thedata, datetime)
            # print("In xarrays daily 1 - area:", thedata)
            # thepolygon = ogrutil.get_polygon_from_wkt(coords)
            # may need to convert the crs...
            # print("thepolygon=", thepolygon)
            # native_crs = self._get_coverage_native_crs(thedata)
            the_xarray_data = gutil.rxr_clip(thedata, theparams, coords)
            print("THEDATA_XARRAY=", the_xarray_data)
    ...
    def _get_coverage_json_grid_range(
        ...
        # if the_var.dtype.name == 'timedelta64[ns]':
        #    the_range["values"] = [
        #        (value / 1000000000.0 if value is not None else 0.0) for value in the_t_var.values.flatten().tolist()]
        # else:
   ...
   MULTIPOINT((38.9 -77),(48.85 2.35),(39.92 116.38),(-35.29 149.1),(51.5 -0.1))
        ...
        try:
            ...
            # print("thedata=", thedata)
            theproperties = self._get_coverage_properties(thedata)
            # print("the_properties=", theproperties)
            # the_id_array = self._form_the_id(thedata)
            # thedata["id_user_"] = the_id_array
            if datetime is not None:
                thedata = self._subset_along_time_dimension(thedata, datetime)
    ...
    def _subset_along_time_dimension(self, the_data, datetime):
        if not datetime:
            return the_data
        print("In _subset_along_time_ datetime=", datetime)
        if '/' in datetime:
            the_times = datetime.split('/')
            if the_times[0] != '..':
                the_data = self._filter_data_xarray_begin(
                    the_data, the_times[0])
                # print("begin = ", the_ctime)
            if the_times[1] != '..':
                the_data = self._filter_data_xarray_end(
                    the_data, the_times[1])
                # print("end = ", the_ctime)
        else:
            the_data = self._filter_data_xarray_equal(
                the_data, datetime)
            # print("time instance = ", the_ctime)
        return the_data

    def _filter_data_xarray_begin(self, the_data, the_time_begin_str):
        try:
            the_time = dateutil.parser.isoparse(the_time_begin_str)
            the_ctime = cftime.DatetimeJulian(
                the_time.year, the_time.month,
                the_time.day, the_time.minute,
                the_time.second)
            the_data = the_data.where(
                the_data[self.time_field] >= the_ctime,
                drop=True)
        except Exception as err:
            Logger.error(err)
        return the_data

    def _filter_data_xarray_end(self, the_data, the_time_end_str):
        try:
            the_time = dateutil.parser.isoparse(the_time_end_str)
            the_ctime = cftime.DatetimeJulian(
                the_time.year, the_time.month,
                the_time.day, the_time.minute,
                the_time.second)
    ...
    def _filter_data_xarray_equal(self, the_data, the_time_str):
        try:
            the_time = dateutil.parser.isoparse(the_time_str)
            the_ctime = cftime.DatetimeJulian(
                the_time.year, the_time.month,
                the_time.day, the_time.minute,
                the_time.second)
    ...
    def _get_daily_files(self, path_pattern):
        the_files = glob.glob(path_pattern, recursive=True)
    ...
    def _get_coverage_json_point_range(
        ...
        the_range["dataType"] = self._get_coveragejson_accepted_data_type(
            the_var.dtype.name)
        the_range["axisNames"] = ["t"]
        the_t_var = the_var.fillna(None)
        the_value_list = the_t_var.values.flatten().tolist()
        the_range["values"] = the_value_list
        the_range["shape"] = [len(the_value_list)]
        return the_range
    # ================coverage-json-point================
    ...
    def _get_coveragejson_accepted_data_type(self, the_dtype_name):
        if self._str_startswith_ignorecase("int", the_dtype_name):
            ...
            return "string"
        elif self._str_startswith_ignorecase("timedelta", the_dtype_name):
            return "float"
        else:
            return "string"
    ...
    def _get_coverage_properties2(self, thedata):
        ...
        # print("thedata.variables=", thedata.variables)
        # print("properties['fields']=", properties['fields'])
    ...
    '''
                ...
                'height': thedata.dims[self.y_field],
                'time': thedata.dims[self.time_field],
                'time_duration': self.get_time_coverage_duration_from_config(the_interval0),
                'bbox_units': 'degrees',
                'resx': np.abs(thedata.coords[self.x_field].values[1]
                               - thedata.coords[self.x_field].values[0]),
                'resy': np.abs(thedata.coords[self.y_field].values[1]
    ...
    '''
    def _get_coverage_properties_from_config(self):
        if self.collection_spec is None:
            return None
        self.x_field = self.provider_def.get('x_field', "x")
        self.y_field = self.provider_def.get('y_field', "y")
        self.time_field = self.provider_def.get('time_field', "t")

        print("from_config time_field=", self.time_field)
        print("from_config x_field=", self.x_field)
        print("from_config y_field=", self.y_field)
        # It would be preferable to use CF attributes to get width
        # resolution etc but for now a generic approach is used to asess
        # all of the attributes based on lat lon vars

        the_extents = self.collection_spec.get("extents", {})
        the_spatial = the_extents.get("spatial",{})
        the_bbox = the_spatial.get("bbox",[-180, -90, 180, 90])
        the_temporal = the_extents.get("temporal",{})
        the_intervals = the_temporal.get("intervals")
        the_interval0 = [None, None]
        if (len(the_intervals)>0):
            the_interval0 = the_intervals[0]

        properties = {
            'bbox': the_bbox,
            'time_range': the_interval0,
            'bbox_crs': 'http://www.opengis.net/def/crs/OGC/1.3/CRS84',
            'crs_type': 'GeographicCRS',
            'x_axis_label': self.x_field,
            'y_axis_label': self.y_field,
        ...
        the_param_names = self.collection_spec.get("parameter_names", {})

        properties['fields'] = [name for name in the_param_names.keys()]
    ...
    @staticmethod
    def _to_timestamp_string(datetime_obj):
        """
        Convenience function to formulate string from various datetime objects
        ...
        value = datetime_obj.strftime('%Y-%m-%d %H:%M:%S.%f')
        return value
    ...
    def get_time_coverage_duration_from_config(self, the_time_range):
        """
        Helper function to derive time coverage duration
        :returns: time coverage duration string
        """
        the_start = dateutil.parser.isoparse(the_time_range[0])
        the_end = dateutil.parser.isoparse(the_time_range[1])

        dur = the_end - the_start
        # thedata[self.time_field][-1] - thedata[self.time_field][0]
        ms_difference = dur.values.astype('timedelta64[ms]').astype(np.double)
