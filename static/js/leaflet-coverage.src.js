...
/**
 * The `change` event, signalling that a different custom coordinate value has been selected.
 * 
 * @typedef {L.Event} CustomAxis#change
 * @property {number} realization The custom coordinate value that has been selected.
 */
 ...
 * 
 * @example <caption>Coverage data layer</caption>
 * new C.CustomAxis(covLayer).addTo(map)
 * // Selecting a vertical coordinate automatically sets the 'custom' property in the layer.
 * // Similarly, when the layer fires an 'axisChange' event with {axis: 'realization'}
 * // the control reflects that change.
 * 
 * @example <caption>Fake layer</caption>
 * var realization = [0,1,2,3,4,5,6]
 * var fakeLayer = {
 *   CustomSlices: realization,
 *   custom: realization[1], // select the second realization step initially
 *   crsCustomAxis: {
 *     name: { 
 *       en: 'realization'
 *     },
 ...
 *   }
 * }
 * var customAxis = new C.CustomAxis(fakeLayer).addTo(map)
 * 
 * // change the height and trigger a manual update
 * fakeLayer.custom = realization[0]
 * customAxis.update()
 * 
 * @extends {L.Layer}
 * 
 * @emits {CustomAxis#change} when a different entry has been selected
 */
 var CustomAxis = function (_L$Layer) {
	inherits(CustomAxis, _L$Layer);
	 ...
	 * @param {object} [options] Control options.
	 * @param {string} [options.position='topleft'] The initial position of the control (see Leaflet docs).
	 * @param {string} [options.title='Custom'] 
	 *   The label to show above the control if `covLayer.crsCustomAxis.name` is missing.
	 * @param {string} [options.templateId] Element ID of an alternative HTML `<template>` element to use. 
	 */
	function CustomAxis(covLayer) {
	  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
	  classCallCheck(this, CustomAxis);
  
	  var _this = possibleConstructorReturn(this, (CustomAxis.__proto__ || Object.getPrototypeOf(CustomAxis)).call(this));
  
	  _this._templateId = options.templateId;
	  _this._covLayer = covLayer;
	  _this._title = options.title || 'realization';
	  _this._position = options.position || 'bottomleft';
	  ...
	  if (covLayer.on) {
		...
		_this._axisListener = function (e) {
		  if (e.axis === 'realization') _this.update();
		};
	  }
	  return _this;
	}
	...
	createClass(CustomAxis, [{
	  ...
	  value: function onAdd(map) {
		...
		var crsCustomAxis = this._covLayer.crsCustomAxis || {};
		var title = crsCustomAxis.name ? covutils.getLanguageString(crsCustomAxis.name) : this._title;
		var unit = covutils.stringifyUnit(crsCustomAxis.unit);
		if (unit) {
		  unit = ' ' + unit;
		}
  
		var choices = [];
		var vals = this._covLayer.customSlices;
		for (var i = 0; i < vals.length; i++) {
		  choices.push({
			value: i.toString(),
			label: vals[i] + unit
		...
		this._dropdown = new Dropdown(choices, {
		  ...
		  position: this._position,
		  title: title,
		  value: this._getCustomIndex().toString()
		}).on('change', function (event) {
		  var i = parseInt(event.value);
		  var val = _this2._covLayer.customSlices[i];
		  _this2._covLayer.realization = val;
		  _this2.fire('change', { realization: val });
		}).addTo(map);
	  }
	  ...
	  key: '_getCustomIndex',
	  value: function _getCustomIndex() {
		var vals = this._covLayer.customSlices;
		var i = vals.indexOf(this._covLayer.realization);
		return i;
	  }
  
	  /**
	   * Triggers a manual update of the custom axis control based on the
...
b"+i+"*=d\
	    ...
	    createClass(_class, [{
	      ...
	      value: function initializePalette() {
	        ...
	        if (this._palette) {
	          ...
	          this._palette = options.palette;
				...
				if (parameter.preferredPalette.colors.length > 2) {
					this._palette = paletteFromObject(parameter.preferredPalette);
				} else {
					this._palette = linearPalette(parameter.preferredPalette.colors);
				}
	        } else if (categories) {
	        ...
	        if (this.parameter.categoryEncoding) {
				let valMap = [];
				for (const [key, value] of this.parameter.categoryEncoding) {
					valMap.push(parseFloat(value[0]));
				  }
			  (function () {
	            // categorical parameter, does not depend on palette extent
	            var max = valMap[valMap.length - 1];
				var min = valMap[0]
	            ...
	            _this2.getPaletteIndex = function (val) {
				  ...
				  let diff = 3000;
                  let idx =-1; 
                  for (let vloop=0;vloop < valMap.length;vloop++) {
                    if  (Math.abs(valMap[vloop] - val) < diff) {
                        diff = Math.abs(valMap[vloop] - val);
                        idx = vloop;
                    } else {
						break; // assumes valMap an ordered array
					}
			  ...
			  this._paletteExtent = []
			  this._paletteExtent.push(valMap[0])
			  this._paletteExtent.push( valMap[valMap.length - 1]) 	
	        }
	      ...
	      set: function set(extent) {
	        if (this.parameter.categoryEncoding) {
			  ...
			  extent = []
			  extent.push(valMap[0])
			  extent.push( valMap[valMap.length - 1]) 	
	        }			  
	...
	var Grid = function (_PaletteMixin) {
	  ...
	  function Grid(cov) {
	    ...
	    _this._axesSubset = { // x and y are not subsetted
		  ...
		  realization: {coordPref: options.realization}		  
	    };
	  ...
	  createClass(Grid, [{
	    ...
	    value: function onAdd(map) {
	      ...
	      this.load().then(function () {
	        ...
	        var custRef = covutils.getReferenceObject(_this2.domain, 'realization');
	        if (custRef && custRef.coordinates.length === 1) {
	          var custRefSys = custRef.system;
	          if (custRefSys.cs && (custRefSys.cs.csAxes || custRefSys.cs.axes)) {
	            _this2.crsCustomAxis = custSys.cs.csAxes ? custRefSys.cs.csAxes[0] : custRefSys.cs.axes[0];
	          }
	        } else {
	          // TODO handle vertical axis part of 3D CRS
	        }			
	    ...
	    set: function set(val) {
	      ...
	      if (!this.domain.axes.has('realization')) {
	        throw new Error('No realization axis found');
	      }
	      var old = this.realization;
	      this._axesSubset.realization.coordPref = val;
	      this._loadCoverageSubset().then(function () {
	        if (old === _this7.realization) return;
	        _this7.redraw();
	        _this7.fire('axisChange', { axis: 'realization' });
	      });
	    }

	    /**
	     * The currently active realization coordinate as a number, 
	     * or undefined if the grid has no realization axis.
	     * 
	     * @type {number|undefined}
	     */
	    ,
	    get: function get() {
	      if (this.domain.axes.has('realization')) {
	        var val = this._subsetDomain.axes.get('realization').values[0];
	        return val;
	      }
	    }

	    /**
	     * The realization slices that make up the coverage, or undefined if the grid has no realization axis .
	     * 
	     * @type {Array<number>|undefined}
	     */
	    ...
	    get: function get() {
	      if (this.domain.axes.has('realization')) {
	        var vals = this.domain.axes.get('realization').values;
	        if (ArrayBuffer.isView(vals)) {
	          // convert to plain Array to allow easier use
	          vals = [].concat(toConsumableArray(vals));
	        }
	...
	var VerticalProfilePlot = function (_L$Popup) {
	  ...
	  function VerticalProfilePlot(coverage) {
	    ...
	    options.maxWidth = options.maxWidth || 400;
	  ...
	  createClass(TimeSeriesPlot, [{
	    ...
	    value: function _getPlotElement(paramKeyGroup) {
	      ...
	      for (var i = 0; i < this._covs.length; i++) {
			...
			var rVals = [];
			var nMem = 1;
			if (this._domains[i].axes.get("realization") != undefined) {
				rVals = this._domains[i].axes.get("realization").values;
				nMem = rVals.length;
			}
	        var vals = this._ranges[i].get(paramKey);
	        var x = [xname];
			var ys = [];
			for (let mem=0; mem < nMem;mem++) {
				var y = [yname];
				...
				for (var j = 0; j < tVals.length; j++) {
					...
					if (this._domains[i].axes.get("realization") != undefined) {
						val = vals.get({ t: j, realization:mem });
					}					
					if (val === null) {
						continue;
					}
	        ...
	        columns.push(x);
			for (let yIdx=0;yIdx < ys.length;yIdx++) {
				columns.push(ys[yIdx]);
				let nKey = yname;
				let rlabel = this._labels[i] ? this._labels[i] : obsPropLabel;
				if (rVals.length > 0) {
					nKey = yname+": "+ rVals[yIdx];
					rlabel = this._labels[i] ? this._labels[i] : obsPropLabel+": "+ rVals[yIdx];
				}
				names[nKey] = rlabel;
				xs[nKey] = xname;				
			}
	      ...
	      el.style.height = height + 'px';
		  this.options.timeFormat = '%a %d %HZ'
	      c3.generate({
	        bindto: el,
	        data: {
	          xs: xs,
